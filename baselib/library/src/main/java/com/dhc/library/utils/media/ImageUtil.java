...
//    }
    ...
    public static String makeThumbnail(Context context, File imageFile) {
        ...
        boolean result = scaleThumbnail(
                imageFile,
                thumbFile,
                DensityUtils.getImageMaxEdge(),
                DensityUtils.getImageMinEdge(),
    ...
    public static int[] getBoundWithLength(int maxSide, Object imageObject, boolean resizeToDefault) {
        ...
        if (String.class.isInstance(imageObject)) {
            ...
            bound = BitmapDecoder.decodeBound(AppContext.get().getResources(), (Integer) imageObject);
            width = bound[0];
            height = bound[1];
    ...
    public static Bitmap getBitmapFromDrawableRes(int res) {
        try {
            return getBitmapImmutableCopy(AppContext.get().getResources(), res);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static int getImageOrientation(String mPaths) {
        int orientation = 0;
        try {
            ExifInterface ei = new ExifInterface(mPaths);
